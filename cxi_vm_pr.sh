...
git rebase $(git merge-base origin/main HEAD) --exec 'set -e; git log -1; SHORT=$(git rev-parse --short HEAD); CWD=$PWD; TESTS="not ok"; COMMIT="not ok"; git log -1 | grep -q "Signed-off-by: "; [[ "$?" -eq "0" ]] && COMMIT="ok"; echo "${COMMIT} - Commit ${SHORT} Signed-off" >> commit-results.tap; make clean && make dist && make -j 8 && ssh -tt localhost "cd $CWD && make check"; ECODE=1; grep -q "not ok" tests/libcxi_test.tap || ECODE=$(($?^1)); [[ "$ECODE" -eq "0" ]] && TESTS="ok"; echo "${TESTS} - Commit ${SHORT} Tests Status" >> commit-results.tap; exit $ECODE'
